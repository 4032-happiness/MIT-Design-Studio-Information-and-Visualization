...
d3.csv("data.csv", function(error, data){
    ...
    var life = country;
    var you = {"Country": "You", "Happiness": happiness, "Life Expectancy/10": life_expectancy, "Social Support": social_support, "Generosity": generosity, "Positive Affect": smile, "Negative Affect": worry};
    data.unshift(you);
    console.log(you);
    showRadar(data);
});
...
var RadarChart = {
  defaultConfig: {
    ...
    minValue: 0,
    radians: 2 * Math.PI,
    color: d3.scale.ordinal().range(['#bf0a0a','#29388d']),
    axisLine: true,
    axisText: true,
    circles: true,
    radius: 5,
  ...
  chart: function() {
    ...
    function setTooltip(tooltip, msg){
      if(msg === false || msg == undefined){
        ...
        .style("width", bbox.width + (padding*2))
        .style("height", bbox.height + (padding*2))
        .attr("rx","5").attr("ry","5")
    ...
    function radar(selection) {
      ...
      var maxValue = 11 //Math.max(cfg.maxValue, d3.max(data, function(d) {
...
//        maxValue -= cfg.minValue;
        ...
        var radius2 = Math.min(1.1*(cfg.w / 2, cfg.h / 2));
        ...
        if(cfg.axisLine || cfg.axisText) {
          ...
          if(cfg.axisLine) {
            ...
            .attr('x1', cfg.w/2)
            .attr('y1', cfg.h/2)
            .attr('x2', function(d, i) { return (cfg.w/2-radius2*1.1)+getHorizontalPosition(i, radius2*1.1, cfg.factor); })
            .attr('y2', function(d, i) { return (cfg.h/2-radius2*1.1)+getVerticalPosition(i, radius2*1.1, cfg.factor); });
          }
